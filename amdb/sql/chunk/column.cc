...
Column::Column(Arena *arena, const ColumnDescriptor *col_desc,
               uint32_t capacity)
    : arena_(arena), desc_(col_desc), capacity_(capacity) {
  data_ = reinterpret_cast<char *>(
      arena_->AllocateBytes(capacity_ * desc_->TypeSize()));
}
...
void Column::SetInt8(uint32_t row_idx, int8_t value) {
  ...
  char *data_ptr = data_ + row_idx * desc_->TypeSize();
...
int8_t Column::GetInt8(uint32_t row_idx) const {
  return *reinterpret_cast<int8_t *>(data_ + row_idx * desc_->TypeSize());
}

uint8_t Column::GetUInt8(uint32_t row_idx) const {
  return *reinterpret_cast<uint8_t *>(data_ + row_idx * desc_->TypeSize());
}
...
int64_t Column::GetInt64(uint32_t row_idx) const {
  return *reinterpret_cast<int64_t *>(data_ + row_idx * desc_->TypeSize());
}

uint64_t Column::GetUInt64(uint32_t row_idx) const {
  return *reinterpret_cast<uint64_t *>(data_ + row_idx * desc_->TypeSize());
}

double Column::GetDouble(uint32_t row_idx) const {
  return *reinterpret_cast<double *>(data_ + row_idx * desc_->TypeSize());
}
