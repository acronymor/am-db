...
Row::Row(Arena* arena, RowDescriptor* desc) : arena_(arena), desc(desc) {
  size_t row_size = desc->cols.size() * sizeof(Tuple);
...
Status Row::SetColValue(uint32_t row_id, uint32_t col_id,
  ...
  if (tuple == nullptr) {
    tuple = reinterpret_cast<char*>(arena_->AllocateBytes(desc->ByteSize()));
    setTuple(row_id, tuple);
  }
  if (value.Type() != desc->GetColumnDesc(col_id)->Type()) {
