...
namespace amdb {
namespace stat {
Status ColumnStatistic::Serialize(ColumnStatisticsProto* output) {
...
Status ColumnStatistic::Deserialize(const ColumnStatisticsProto& input) {
...
Status MetaStatistic::Serialize(StatisticsMetaProto* output) {
  output->set_db_id(this->db_id);
  output->set_table_id(this->table_id);
...
Status MetaStatistic::Deserialize(const StatisticsMetaProto& input) {
  this->db_id = input.db_id();
  this->table_id = input.table_id();
