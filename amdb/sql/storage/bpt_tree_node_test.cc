...
namespace amdb {
namespace storage {
class BplusTreeNodeTest : public testsuite::KvDataTest {
 protected:
  void SetUp() override {
    KvDataTest::SetUp();
  ...
  void TearDown() override { KvDataTest::TearDown(); }

 protected:
  TreeCtx* tree_ctx_;
  TreeCtx::Schema schema_ = {0, 0, 0};
};

TEST_F(BplusTreeNodeTest, Serialize) {
  BptNodeProto root_proto1;
  root_proto1.set_id(0);
