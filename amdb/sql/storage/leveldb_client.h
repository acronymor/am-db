...
class LevelDbClient : public KvStorageAPI {
 public:
  ...
  Status MPutKV(const std::vector<std::string>& keys, const std::vector<std::string>& values) override;

  Status MPutKV(const std::unordered_map<std::string, std::string>& kv_map) override;

  Status GetKV(const std::string& key, std::string* value) override;

  Status MGetKV(const std::vector<std::string>& keys, std::vector<std::string>* values) override;

  Status DelKV(const std::string& key) override;

  Status MDelKV(const std::vector<std::string>& keys) override;

  Status Incrby(const std::string& key, uint64_t step, uint64_t* new_val) override;
 ...
 private:
  explicit LevelDbClient(std::unique_ptr<leveldb::DB>&& db) : db_(std::move(db)){};
